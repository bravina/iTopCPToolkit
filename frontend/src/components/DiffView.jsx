...
export default function DiffView({ configA, blocks, onClose }) {
    ...
    return (
      <div className="flex flex-col flex-1 overflow-hidden">
        <div className="px-4 py-2 bg-slate-100 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex items-center gap-3">
          <span className="text-sm font-semibold text-slate-800 dark:text-slate-200">Diff Mode</span>
          <span className="text-xs text-slate-600 dark:text-slate-400">Load a second config to compare against the current one</span>
          <button
            type="button"
            onClick={onClose}
            className="ml-auto text-xs text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 px-2 py-1 rounded hover:bg-slate-200 dark:hover:bg-slate-700"
          >
            ✕ Cancel diff
          </button>
        </div>
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
      {/* Diff toolbar */}
      <div className="px-4 py-2 bg-slate-100 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex items-center gap-4 text-xs shrink-0">
        <span className="font-semibold text-slate-800 dark:text-slate-200">Diff</span>
        ...
        <div className="flex gap-3 ml-2">
          {stats.added   && <span className="text-emerald-600 dark:text-emerald-400">+{stats.added} added</span>}
          {stats.removed && <span className="text-red-600 dark:text-red-400">−{stats.removed} removed</span>}
          {stats.changed && <span className="text-yellow-600 dark:text-yellow-400">~{stats.changed} changed</span>}
